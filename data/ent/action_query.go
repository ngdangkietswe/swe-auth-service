...
func (aq *ActionQuery) QueryPermissions() *PermissionQuery {
...
func (aq *ActionQuery) WithPermissions(opts ...func(*PermissionQuery)) *ActionQuery {
	query := (&PermissionClient{config: aq.config}).Query()
...
func (aq *ActionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Action, error) {
	...
	if query := aq.withPermissions; query != nil {
		if err := aq.loadPermissions(ctx, query, nodes,
			func(n *Action) { n.Edges.Permissions = []*Permission{} },
			func(n *Action, e *Permission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ActionQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Action, init func(*Action), assign func(*Action, *Permission)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(action.PermissionsTable)
		s.Join(joinT).On(s.C(permission.FieldID), joinT.C(action.PermissionsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(action.PermissionsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(action.PermissionsPrimaryKey[0]))
