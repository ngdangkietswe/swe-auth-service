...
func (pq *PermissionQuery) QueryUsersPermissions() *UsersPermissionQuery {
	query := (&UsersPermissionClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withUsersPermissions: pq.withUsersPermissions.Clone(),
...
func (pq *PermissionQuery) WithUsersPermissions(opts ...func(*UsersPermissionQuery)) *PermissionQuery {
	query := (&UsersPermissionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withUsersPermissions = query
	return pq
}
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withUsersPermissions; query != nil {
		if err := pq.loadUsersPermissions(ctx, query, nodes,
			func(n *Permission) { n.Edges.UsersPermissions = []*UsersPermission{} },
			func(n *Permission, e *UsersPermission) {
				n.Edges.UsersPermissions = append(n.Edges.UsersPermissions, e)
			}); err != nil {
			return nil, err
		}
	}
